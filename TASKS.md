...
## 待开始
<!-- 新任务添加到这里 -->
  - **背景**：Boss 发现无法设置第三方模型（智谱、中转 API）
  - **问题**：模型下拉框只能从预定义列表选择，无法输入自定义模型名称
  - **需求**：
    1. 模型选择框改为可编辑输入框（支持自定义模型名称）
    2. 或添加"自定义模型"选项到列表
  - **涉及文件**：
    - `SettingsDialog.tsx`: 前端模型选择组件
    - `backend/api/settings.py`: 后端 AVAILABLE_MODELS 列表
  - **优先级**：高（阻塞用户使用第三方 API）
